...
class UsuarioModel
{
    ...
    public function registrarUsuario($nombres,$correo,$password, $rol)
    {
        $sql = $this->conexion->query("INSERT INTO usuarios (nombre, email,password ,rol ) VALUES ('$nombres','$correo','$password','$rol')");
        if ($sql) {
   ...
   public function actualizarUsuario($id, $nombres_apellidos, $correo, $rol, $estado)
    {
        $sql = $this->conexion->query("UPDATE usuarios SET nombre='$nombres_apellidos',email='$correo',rol='$rol',estado ='$estado' WHERE id='$id'");
        return $sql;
    }
